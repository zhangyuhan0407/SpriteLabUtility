...
extension Json {
    ...
    public var int: Int? {
        guard case let .number(double) = self where double.truncatingRemainder(dividingBy: 1) == 0 else {
            return nil
        }
...
extension Json {
    public subscript(index: Int) -> Json {
        assert(index >= 0)
        guard let array = self.array where index < array.count else { return JSON.null }
        return array[index]
    }

    public subscript(key: String) -> Json {
        get {
            guard let dict = self.object else { return JSON.null }
